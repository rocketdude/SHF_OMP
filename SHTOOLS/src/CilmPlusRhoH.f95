...
!			mass:		Mass of planet.
!			rho:		Input grid of the density with same dimensions as GRIDIN.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	if (size(gridin(1,:)) < nlong .or. size(gridin(:,1)) < nlat .or. size(rho(1,:)) < nlong .or. size(rho(:,1)) < nlat) then
		print*, "Error --- CilmPlusRHOH"
		if (gridtype == 2) then
			print*, "GRIDIN and RHO must be dimensioned as (LMAX+1, 2*LMAX+1) where LMAX is ", lmax
			print*, "Input dimension of GRIDIN is ", size(gridin(1,:)), size(gridin(:,1))
			print*, "Input dimension of RHO is ", size(rho(1,:)), size(rho(:,1))
			stop
		elseif (gridtype == 3) then 
			print*, "GRIDIN and RHO must be dimensioned as (N, N) where N is ", n
			print*, "Input dimension of GRIDIN is ", size(gridin(1,:)), size(gridin(:,1))
			print*, "Input dimension of RHO is ", size(rho(1,:)), size(rho(:,1))
			stop
		elseif (gridtype == 4) then 
			print*, "GRIDIN and RHO must be dimensioned as (N, 2N) where N is ", n
			print*, "Input dimension of GRIDIN is ", size(gridin(1,:)), size(gridin(:,1))
			print*, "Input dimension of RHO is ", size(gridin(1,:)), size(gridin(:,1))
			stop
		endif
	endif
			...
			case(1)
				if (present(plx)) then
					call SHExpandGLQ(cilmn(1:2,1:lmax+1,1:lmax+1), lmax, grid(1:nlat,1:nlong), &
						w, plx = plx, norm = 1, csphase = 1, lmax_calc = 0)
				else
					call SHExpandGLQ(cilmn(1:2,1:lmax+1,1:lmax+1), lmax, grid(1:nlat,1:nlong), &
						w, zero=zero, norm = 1, csphase = 1, lmax_calc = 0)
				endif
			case(2); call SHExpandDH(grid(1:nlat,1:nlong), n, cilmn(1:2,1:lmax+1,1:lmax+1), &
				lmax_dh, norm = 1, sampling = 1, csphase = 1, lmax_calc = 0)
			case(3); call SHExpandDH(grid(1:nlat,1:nlong), n, cilmn(1:2,1:lmax+1,1:lmax+1), &
				lmax_dh, norm = 1, sampling = 2, csphase = 1, lmax_calc = 0)
		end select
